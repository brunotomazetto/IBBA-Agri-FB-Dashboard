...
GRUPO_PRODUTIVIDADE = "697311317415952387"
ESTADO_MT           = "51"
TIPO_LOCALIDADE     = "1"
...
def get_indicadores(token, cadeia_id):
    todos, page = [], 1
    safra_ids = [s[0] for s in SAFRAS]
    while True:
        resp = requests.post(
            f"{API_INDICADORES}?nome=&pageSize=100&page={page}",
            json={"nome": "", "pageSize": 100, "page": page,
                  "cadeia": [cadeia_id], "grupo": [GRUPO_PRODUTIVIDADE],
...
def get_dados(token, cadeia_id, ind_id, safra_id, inicio, fim):
    resp = requests.post(API_DADOS,
        json={"cadeia": [cadeia_id], "grupo": [GRUPO_PRODUTIVIDADE],
...
for cultura, cadeia_id in CULTURAS.items():
    ...
    for ind in indicadores:
        ...
        for safra_id, safra_desc in SAFRAS:
...
# Resumo
print("\n=== PRODUTIVIDADE INSERIDA ===")
for row in conn.execute("""
    SELECT cultura, indicador_nome, COUNT(*), MIN(data_referencia), MAX(data_referencia)
    FROM historico WHERE grupo='PRODUTIVIDADE'
    GROUP BY cultura, indicador_nome ORDER BY cultura
"""):
    print(f"  {row[0]} | {row[1]} | {row[2]} registros | {row[3]} → {row[4]}")
